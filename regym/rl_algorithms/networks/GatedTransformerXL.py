...
class RelMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
        ...
        def forward(self, w, r, attn_mask=None, mems=None):
            return
class MultiHeadAttention(RelMultiHeadAttn):
...
# variable names follow the notation from Gated-Recurrent-Unit-type gating
class GRUGate(nn.Module):
    ...
    def __init__(self, d_model):
        ...
        # TODO:  intitialize bias of linear_w_z to -3
        self.linear_w_r = nn.Linear(d_model, d_model, bias=False)
        self.linear_u_r = nn.Linear(d_model, d_model, bias=False)
        self.linear_w_z = nn.Linear(d_model, d_model)  ### Giving bias to this layer (b_g : initialize negative)
    ...
    def init_bias(self):
        with torch.no_grad():
            self.linear_w_z.bias.fill_(
                -2)  # setting this bias # -2 is the setting used in the paper stable transformers
    ...
    def forward(self, x, y):
        ...
        h_hat = torch.tanh(self.linear_w_g(y) + self.linear_u_g(r * x))  # elementwise multiplication of r and x
...
class PositionwiseFF(nn.Module):
    ...
    def forward(self, inp):
        ##### positionwise feed-forward (original transformer)
...
class GatedTransformerLayer(nn.Module):
    def __init__(self,n_head, d_model, d_head, d_inner, dropout):
        ...
        self.dec_attn = MultiHeadAttention(n_head, d_model,
                                           d_head, dropout)
...
class GatedTransformerXL(nn.Module):
    ...
    # qlen number of characters in one example
    # mlen is memory size
    def _update_mems(self, hids, mems, qlen, mlen):
        ...
        with torch.no_grad():
            new_mems = []
            end_idx = mlen + max(0,
                                 qlen - 0 - self.ext_len)  # ext_len looks to usually be 0 (in their experiments anyways

            # beg_idx to 0  to use all memory,change otherwise
